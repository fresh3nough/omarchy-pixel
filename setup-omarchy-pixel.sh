...
# Step 7: Install launcher APK and shortcuts
log "7/7 Installing Omarchy launcher and shortcuts"
[ -f "$HOME_DIR/configure-termux-x11.sh" ] && "$HOME_DIR/configure-termux-x11.sh" || true

# Install launcher APK if available
if [ -f "/sdcard/omarchy-pixel/Omarchy.apk" ]; then
  log "Installing Omarchy launcher APK..."
  pm install -r "/sdcard/omarchy-pixel/Omarchy.apk" 2>/dev/null || {
    log "APK install failed - user will need to install manually"
...
mkdir -p "$HOME_DIR/.shortcuts"
cat > "$HOME_DIR/.shortcuts/Omarchy.sh" << 'WIDGET_SHORTCUT'
#!/data/data/com.termux/files/usr/bin/bash
# Omarchy launcher for Termux:Widget
export PATH="${PREFIX:-/data/data/com.termux/files/usr}/bin:$PATH"
H="${HOME:-/data/data/com.termux/files/home}"

# Try home directory first (faster)
...
echo "ERROR: Omarchy start script not found" >&2
echo "Expected: $H/start-omarchy-fullscreen.sh" >&2  
echo "Or: /sdcard/omarchy-pixel/start-omarchy-fullscreen.sh" >&2
exit 1
WIDGET_SHORTCUT
chmod 755 "$HOME_DIR/.shortcuts/Omarchy.sh"

# Copy launcher script to home directory for fastest access
if [ -f "/sdcard/omarchy-pixel/start-omarchy-fullscreen.sh" ]; then
  cp -f "/sdcard/omarchy-pixel/start-omarchy-fullscreen.sh" "$HOME_DIR/"
  chmod 755 "$HOME_DIR/start-omarchy-fullscreen.sh"
fi

# Also copy the Omarchy.sh shortcut itself to home
cp -f "$HOME_DIR/.shortcuts/Omarchy.sh" "$HOME_DIR/"
chmod 755 "$HOME_DIR/Omarchy.sh"

log "✓ Omarchy launcher APK and shortcuts configured"
log "  • APK installed: com.omarchy.launcher" 
log "  • Widget shortcut: ~/.shortcuts/Omarchy.sh"
log "  • Home launcher: ~/Omarchy.sh"
log "  • Home start script: ~/start-omarchy-fullscreen.sh"
